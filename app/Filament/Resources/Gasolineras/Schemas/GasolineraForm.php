                    ...
                    ->schema([
                        ...
                        TinyEditor::make('texto_inicio')
                            ...
                            ->afterStateHydrated(function ($state, $set, $record) {
                                $cleanState = trim(strip_tags($state ?? '', '<img><iframe>'));
                                if ($cleanState === '' || $state === '<p></p>') {
                                    $nombre = $record?->gasolinera?->Nombre ?? 'nuestra estación';
                                    $set('texto_inicio', "<p>Bienvenido a <strong>{$nombre}</strong>, parte de la red de estaciones de servicio <strong>Utrecar</strong>. Nos esforzamos diariamente para brindarte una experiencia de repostaje excepcional, combinando combustibles de última generación con una atención al cliente cercana y profesional.</p><p>En nuestras instalaciones encontrarás carburantes de alta gama formulados con aditivos de última generación que optimizan el rendimiento del motor, protegen los componentes frente a la corrosión y reducen el consumo y las emisiones contaminantes. Todo ello garantizando siempre precios altamente competitivos y la máxima fiabilidad en el suministro.</p>");
                                }
                            }),
                        ...
                        TinyEditor::make('quienes_somos')
                            ...
                            ->afterStateHydrated(function ($state, $set, $record) {
                                $cleanState = trim(strip_tags($state ?? '', '<img><iframe>'));
                                if ($cleanState === '' || $state === '<p></p>') {
                                    $set('quienes_somos', "<p><strong>Utrecar</strong> es una red de estaciones de servicio comprometida con ofrecer el máximo estándar de calidad y un servicio excelente. Nos basamos en tres pilares fundamentales: <strong>Combustibles Premium</strong>, <strong>Atención Cercana</strong> e <strong>Instalaciones Confortables</strong>.</p><p>Nuestra misión es suministrar energía eficiente para la movilidad de personas y flotas profesionales. Controlamos de forma rigurosa cada paso de la cadena de distribución del combustible para asegurar que el carburante entregado a tu vehículo posea las mejores propiedades protectoras y de rendimiento.</p><p>Apostamos firmemente por el desarrollo local y el respeto al medio ambiente, adaptando constantemente nuestros servicios para ser tu punto de parada de confianza y hacer de cada repostaje una parada cómoda y segura.</p>");
                                }
                            }),
