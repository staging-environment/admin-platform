...
class ManageEmpleadoNotificaciones extends Component
{
    ...
    public $resolucion_cierre = 'Amonestación';
    public $dias_suspension;
    public $archivo;
    ...
    public function updatedTipo($value)
    {
        if ($value === 'Modificación sustancial del contrato') {
            ...
            $this->gravedad = 'Leve';
        } elseif ($value === 'Cierre expediente disciplinario') {
            $this->resolucion_cierre = 'Amonestación';
            $this->dias_suspension = null;
        }
    }

    public function updatedResolucionCierre($value)
    {
    ...
    {
        $rules = [
            'tipo' => 'required|in:Modificación sustancial del contrato,Apertura Expediente disciplinario,Cierre expediente disciplinario',
        ...
        } elseif ($this->tipo === 'Cierre expediente disciplinario') {
            $rules['resolucion_cierre'] = 'required|in:Amonestación,Suspensión de empleo y sueldo,Despido disciplinario';
            if ($this->resolucion_cierre === 'Suspensión de empleo y sueldo') {
                $rules['dias_suspension'] = 'required|integer|min:1';
            }
        }
        ...
        $this->validate($rules, [
            ...
            'resolucion_cierre.required' => 'La resolución de cierre es obligatoria.',
            'dias_suspension.required' => 'El número de días de suspensión es obligatorio.',
            'dias_suspension.min' => 'Los días de suspensión deben ser al menos 1.',
        ...
        // Crear notificación
        $notificacion = EmpleadoNotificacion::create([
            ...
            'resolucion_cierre' => $this->tipo === 'Cierre expediente disciplinario' ? $this->resolucion_cierre : null,
            'dias_suspension' => ($this->tipo === 'Cierre expediente disciplinario' && $this->resolucion_cierre === 'Suspensión de empleo y sueldo') ? (int)$this->dias_suspension : null,
            'file_path' => $path,
        ]);
        ...
        if ($this->tipo === 'Cierre expediente disciplinario' && $this->resolucion_cierre === 'Despido disciplinario') {
            $empleado->update([
                'estado' => 'Baja',
                'motivo_baja' => 'Despido disciplinario',
                'observaciones_baja' => 'Cierre de expediente disciplinario con resolución de despido.',
                'fecha_baja' => $this->fecha_comunicacion,
                'documento_baja_path' => $path,
            ]);
        }

        // Reset form
        $this->reset(['archivo', 'dias_suspension']);
        $this->fecha_comunicacion = now()->format('Y-m-d');
        $this->fecha_efecto = now()->format('Y-m-d');
