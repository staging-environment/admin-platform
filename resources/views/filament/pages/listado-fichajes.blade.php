<x-filament-panels::page>
    ...
    <div class="space-y-6">
        ...
        <!-- Header Card -->
        <div class="bg-white dark:bg-gray-900 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-white/5 relative overflow-hidden">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <x-heroicon-o-user-group class="w-6 h-6 text-amber-500" />
                        Control General de Fichajes de Empleados
                    </h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Historial consolidado de registros de jornada, ubicaciones y horas de todos los trabajadores.
                    </p>
                </div>
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    <button 
                        ...
                        class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all focus:ring-2 focus:ring-red-500 focus:outline-none disabled:opacity-50"
                    >
                        <x-heroicon-o-arrow-down-tray class="w-4 h-4" wire:loading.remove wire:target="exportPdf" />
                        <svg wire:loading wire:target="exportPdf" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span>Exportar PDF</span>
                    </button>
                    <span class="text-xs font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800/40 px-3 py-1.5 rounded-xl">
                        {{ $todosLosFichajes->total() }} registros totales
                    </span>
                </div>
            </div>

            <!-- Filters Section -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6 mb-6 bg-gray-50 dark:bg-gray-950/20 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
                <!-- Date Filter "Desde" -->
                <div>
                    <label for="filterDateFrom" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Desde</label>
                    <div class="relative">
                        <input type="date" id="filterDateFrom" wire:model.live="filterDateFrom" class="w-full text-sm rounded-xl border-gray-300 dark:border-white/10 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:border-amber-500 focus:ring-amber-500 shadow-sm py-2 px-3 pr-8" />
                        @if($filterDateFrom)
                            <button type="button" wire:click="$set('filterDateFrom', '')" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="date" id="filterDateTo" wire:model.live="filterDateTo" class="w-full text-sm rounded-xl border-gray-300 dark:border-white/10 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 focus:border-amber-500 focus:ring-amber-500 shadow-sm py-2 px-3 pr-8" />
                        @if($filterDateTo)
                            <button type="button" wire:click="$set('filterDateTo', '')" class="absolute inset-y-0 right-0 pr-2.5 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                            </button>
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                                <td class="py-2 px-4 font-bold text-gray-900 dark:text-white uppercase text-xs">
                                    <div class="flex items-center gap-2">
                                        @include('filament.components.fichaje-alerts-badge', ['fichaje' => $fichaje])
                                        <span class="truncate">{{ $fichaje->empleado ? mb_strtoupper($fichaje->empleado->apellidos ?? '') : 'N/A' }}</span>
                                    </div>
                                </td>
                                <td class="py-2 px-4 font-bold text-gray-900 dark:text-white uppercase text-xs">
                                ...
                                <td class="py-2 px-4 text-gray-600 dark:text-gray-400">
                                    @php
                                        $isEntradaDescuadre = $fichaje->isEntradaDescuadre();
                                        ...
                                        if ($isEntradaDescuadre) {
                                            $tooltipEntrada .= ' (⚠️ Descuadre > 5 min)';
                                        }
                                    @endphp
                                    <div class="flex items-center gap-1.5">
                                        <span 
                                            class="inline-flex items-center px-2 py-0.5 rounded {{ $isEntradaDescuadre ? 'bg-red-50 text-red-700 border border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800/40 font-bold' : 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400' }} font-mono text-xs font-bold w-fit cursor-help transition-all hover:opacity-80"
                                            @if($tooltipEntrada)
                                        ...
                                        >
                                            ...
                                            @if($isEntradaDescuadre)
                                                <span class="ml-1 text-[10px]" title="Descuadre > 5 min">⚠️</span>
                                            @endif
                                        </span>
                                ...
                                <td class="py-2 px-4 text-gray-600 dark:text-gray-400">
                                    @if($fichaje->hora_salida)
                                        @php
                                            $isSalidaDescuadre = $fichaje->isSalidaDescuadre();
                                            ...
                                            if ($isSalidaDescuadre) {
                                                $tooltipSalida .= ' (⚠️ Descuadre > 5 min)';
                                            }
                                        @endphp
                                        <div class="flex items-center gap-1.5">
                                            <span 
                                                class="inline-flex items-center px-2 py-0.5 rounded {{ $isSalidaDescuadre ? 'bg-red-50 text-red-700 border border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800/40 font-bold' : 'bg-orange-50 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400' }} font-mono text-xs font-bold w-fit cursor-help transition-all hover:opacity-80"
                                                @if($tooltipSalida)
