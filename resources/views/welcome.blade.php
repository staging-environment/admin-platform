    ...
    <div class="relative w-full overflow-hidden bg-slate-900 slider-container-dynamic" 
        ...
        <header class="absolute top-0 left-0 right-0 z-20 max-w-7xl mx-auto px-6 py-6 flex justify-end items-center">
            ...
            <nav class="flex gap-3 items-center">
                @if (Route::has('login'))
                    @auth
                        @if(auth()->user()->email === 'jarodriguezbonilla@gmail.com' || auth()->user()->id === 1 || auth()->user()->can('gestion_portada'))
                    ...
                    @endauth
                @endif
                <a href="{{ route('offers.index') }}" class="ml-3 text-xs font-bold bg-gradient-to-r from-purple-600 to-pink-600 text-white backdrop-blur-md px-5 py-2.5 rounded-full hover:from-purple-700 hover:to-pink-700 transition-all duration-300 shadow-lg">Trabaja con nosotros</a>
            </nav>
        </header>
        ...
        <div x-show="false" 
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <form action="{{ route('home.contacto') }}" method="POST" class="lg:col-span-7 space-y-6">
                    ...
                    <div id="security-wrapper-home"></div>
                    <script>
                        setTimeout(function() {
                            var container = document.getElementById('security-wrapper-home');
                            if (container) {
